...
vvs.data <- setkey(rbindlist(lapply(sectors,function(sec){
...
}))[,':='(btm=(tot.assets-tot.liab)/as.numeric(market.cap),size=log(as.numeric(market.cap)),debt.to.eq=st.debt/(tot.assets-tot.liab),accruals=total.accruals(curr.assets,cash,curr.liab,st.debt,taxes,depr,tot.assets)),by=list(Stock,Sector)],q.id,Stock)


###Broker specific variables load q.data from cache
load('cache/q.data.RData')
broker.vvs <- acast(melt(unique(q.data[,broker.vvs.f(PT,priceAdj),by=list(q.id,Stock)],by=c('q.id','Stock')),id.vars = c('q.id','Stock'),measure.vars = c('uncertainty','assym','dispersion')),q.id~Stock~variable)
...
all.vvs <- all.macro[setkey(vvs.data[,list(q.id,Stock,Sector,btm,size,debt.to.eq,accruals)][unique(stock.returns[,list(q.id,Stock,s.vol)],by=c('q.id','Stock')),allow.cartesian=T],q.id,Sector)]
array.all.vvs <- acast(unique(melt(all.vvs,id.vars = c('q.id','Stock'),measure.vars =  c('btm','size','debt.to.eq','accruals','s.vol','sec.ret','growth.gnp','inf.rate','ret.vix','t.bill')),by=c('q.id','Stock','variable')),q.id~Stock~variable)
