...
###Core.dt: 1. Brokers, that follow a stock for 12 qtrs
###2.Stocks with more than 3 brokers
###3.Stocks with more then 8 quarters of history
...
baselines.dt<- core.dt[,merge(setkey(quarters,q.id),.SD,all=T),by=list(Broker,Stock),.SDcols=c('q.id','Broker','Stock','true')][,.(q.id,Broker,Stock,true)]

pt.new.tmp <- baselines.dt[,naive:=c(NA,head(true,-1)),by=.(Broker,Stock)][,mean.rank:=rollapplyr(naive,seq_len(length(naive)),mean,na.rm=T,partial=T),by=.(Broker,Stock)][,default:=rank(mean.rank,na.last = 'keep'),by=.(q.id,Stock)]

pt.new.dt <- setkey(melt(pt.new.tmp,id.vars = c('q.id','Stock','Broker'),measure.vars = baselines,value.name = 'rank',variable.name = 'Method'),q.id,Stock,Broker,Method)[!is.na(q.id)]



pt.new <- acast(pt.new.dt,q.id~Broker~Stock~Method,value.var='rank')
...
#baseline.rankings <- baseline.rankings.f(pt.new[,,stocks],1)
#dimnames(baseline.rankings)[[1]] <- dimnames(pt.new)[[1]]

stock.vvs <- vvs.combine(stocks,broker.vvs,array.all.vvs,t=dim(pt.new)[1])

pred.r<- predict.ranking.script.f(methods,stocks,stock.vvs,pt.new[,,,baselines[1]],rank.parameters)
#dimnames(pred.r)[[2]] <- dimnames(pt.new)[[1]][4:43]
all.rankings <- abind(pt.new[(dim(pt.new)[1]-dim(pred.r)[2]+1):dim(pt.new)[1],,stocks,],aperm(pred.r,c(2,1,4,3)),along=4)
#all.rankings <- abind(pt.new[2:dim(pt.new)[1],,stocks,],aperm(pred.r,c(2,1,4,3)),along=4)
