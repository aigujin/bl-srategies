...
brok.tper <- acast(unique(core.dt,by=c('Broker','q.id','Stock'),fromLast = T),q.id~Broker~Stock,value.var=c('b.view'))



##core rankings
pt.new <- acast(unique(core.dt,by=c('Broker','q.id','Stock'),fromLast = T),q.id~Broker~Stock,value.var='rank')
pt.stocks=intersect(dimnames(brok.tper)[[3]],market.set[,Stock])


#pt.stocks=intersect(dimnames(pt.new)[[3]],market.set[,Stock])
pt.baseline.rankings <- baseline.rankings.f(pt.new[,,pt.stocks],1)
pt.all.s <- findIntersect(pt.baseline.rankings,brok.tper,3)
pt.all.b <- findIntersect(pt.baseline.rankings,brok.tper,2)
percentile <- 0.05
pt.trunk.brok.tper <- truncate.f(brok.tper[,,pt.all.s],percentile)
...
eps.new <- acast(unique(complete.dt,by=c('Broker','calQ','Stock'),fromLast = T),calQ~Broker~Stock,value.var='rank')

eps.all.s <- findIntersect(eps.new[dimnames(pt.new)[[1]],,],pt.trunk.brok.tper,3)
eps.all.b <- findIntersect(eps.new[dimnames(pt.new)[[1]],,],pt.trunk.brok.tper,2)

eps.baseline.rankings <- baseline.rankings.f(eps.new[dimnames(pt.new)[[1]],eps.all.b,eps.all.s],1)


eps.list.rank <- alply(bl.script.f(eps.all.s,eps.baseline.rankings[3:42,,,],pt.trunk.brok.tper[,eps.all.b,eps.all.s],t=dim(eps.baseline.rankings[3:42,,,])[1]),3,.dims=T)

eps.rank.views <- setnames(data.table(reshape2::melt(eps.list.rank)),c('q.num','Stock','View','Method'))[,q.id:=dimnames(pt.baseline.rankings)[[1]][3:42],by=list(Stock,Method)][,year:=format(as.yearqtr(q.id),'%Y')]
...
meanTper <-acast(unique(setkey(core.dt[,meanTper:=median(b.view),by=list(q.id,Stock)],Stock)[pt.all.s],by=c('Stock','q.id')),q.id~Stock,value.var='meanTper')

trunk.meanTper <- truncate.f(meanTper,percentile)
cons.list.rank <- alply(abind(true=trunk.meanTper[3:43,],naive=trunk.meanTper[2:42,],default=rollapplyr(trunk.meanTper[2:42,],seq_len(nrow(trunk.meanTper[2:42,])),mean,na.rm=T),along=3),3,.dims=T)
