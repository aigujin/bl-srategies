...
eps.all.rankings <- bl.all.rankings
...
#load('cache/all.rankings.RData')
all.s <- intersect(findIntersect(all.rankings,brok.tper,3),dimnames(eps.all.rankings)[[3]])
all.b <- intersect(findIntersect(all.rankings,brok.tper,2),dimnames(eps.all.rankings)[[2]])
...
list.rank <- alply(bl.script.f(all.s,eps.all.rankings[,all.b,all.s,],trunk.brok.tper,t=dim(all.rankings)[1]),3,.dims=T)

rank.conf <- buildOmega(apply(all.rankings[,all.b,all.s,],c(1,3),function(s){apply(s,2,evaluation.simple,s[,'true'],'s','p' ) }))


pt.eps.rank.views <- setnames(data.table(melt(list.rank)),c('q.num','Stock','View','Method'))[,q.id:=dimnames(all.rankings)[[1]],by=list(Stock,Method)][,year:=format(as.yearqtr(q.id),'%Y')]
cache('pt.eps.rank.views')
...
pt.eps.opt.w <- opt.w.f(c('last','ma','w.ma'),list.rank,c(list.rank[1:3],list.non.rank),case.1.non.r.conf,non.r.conf,case='1',0.02,delta)
cache('pt.eps.opt.w')
