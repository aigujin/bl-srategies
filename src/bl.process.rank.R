opt.w<- rbindlist(lapply(c('last','ma','w.ma'),function(agg){
  ...
  #list.conf <- alply(rank.conf[,,,agg],2,.dims=T)
  rbindlist(lapply(names(list.views),function(type){
    rbindlist(script.weights.env.f(type,market.list[m.period],list.views[[type]][bl.period,],list.conf[[type]][bl.period,],0.02,2.5))[,Aggregation:=agg]
  }))
}))



bl.results <- unique(opt.w,by=c('q.id','Method','Aggregation'))[,list(Quarters=as.yearqtr(Quarters),Method,Aggregation,port.ret,n.views,q.id)]
  
setkey(market.set,Quarters)
market.info <- unique(market.set[,':='(port.ret=sum(mw*ex.ret),Method='Market',n.views=.N),by=Quarters])[m.period,list(Quarters=as.yearqtr(Quarters),Method,port.ret,n.views)][,q.id:=.I]

bl.results <- rbind(bl.results,bl.results[,as.list(market.info),by=Aggregation],use.names=T)

bl.results[,c('cum.ret','ann.ret','ann.sd','meanViews'):=cum.ret.f(port.ret,n.views),by=list(Method,Aggregation)][,ann.sr:=ann.ret/ann.sd]
setkey(bl.results,Method,Aggregation)
bl.results[,c('cum.ret','ann.ret','ann.sd','meanViews'):=cum.ret.f(port.ret,n.views),by=list(Method,Aggregation)][,ann.sr:=ann.ret/ann.sd]
setkey(bl.results,Method,Aggregation)
to.bl <- rbindlist(lapply(2:length(bl.period),function(b){
  beg <- setkey(opt.w[q.id==b,list(Stock,opt.w),by=list(Method,Aggregation)],Stock,Method,Aggregation)
  end <- setkey(opt.w[q.id==b-1,list(Stock,opt.w),by=list(Method,Aggregation)],Stock,Method,Aggregation)
  end[beg][,turnover.f(opt.w,i.opt.w),by=list(Method,Aggregation)]
  }))[,Ave.TO:=mean(V1),by=list(Method,Aggregation)]

to.market <- rbindlist(lapply(2:length(m.period),function(b){
beg <- setkey(market.set[,q.num:=.GRP,by=q.id][q.num==m.period[b],list(Method,Stock,mw)],Stock,Method)
end <- setkey(market.set[,q.num:=.GRP,by=q.id][q.num==m.period[b-1],list(Method,Stock,mw)],Stock,Method)
end[beg,allow.cartesian=T][,turnover.f(mw,i.mw),by=Method]}))[,Ave.TO:=mean(V1),by=list(Method)]


final.bl <- setkey(bl.results,Method,Aggregation)[setkey(rbind(to.bl[,list(Method,Aggregation,Ave.TO)],to.bl[,as.list(unique(to.market,by='Ave.TO')[,list(Method,Ave.TO)]),by=Aggregation],use.names=T),Method,Aggregation),allow.cartesian=T]
