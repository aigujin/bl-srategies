...
brok.full.stat.pt <- rbind(setkey(core.dt[,.N,by=.(Stock,q.id,year,type)][,list(min=min(N),mean=mean(N),median=as.character(median(N)),max=max(N),st.dev=sd(N)),by=.(year,type)],year),setnames(cbind('Total',(core.dt)[,.N,by=.(Stock,q.id,year,type)][,list(min=min(N),mean=mean(N),median=as.character(median(N)),max=max(N),st.dev=sd(N)),by=type]),1,'year'))
...
tab.r <- round(cbind(pt.cont.tab[,,'all','t'],Total=apply(pt.cont.tab[,,1,1],1,sum)),3)*100
...
tab.r <- round(cbind(pt.cont.tab[,,'all','t+4'],Total=apply(pt.cont.tab[,,1,2],1,sum)),3)*100
rownames(tab.r) <- c('&$top$','&$middle$','&$bottom$')
#colnames(tab.r) <- c('$top$','$middle$','$bottom$','Sum')
...
tab.r <- round(cbind(eps.cont.tab[,,'all','t'],Total=apply(eps.cont.tab[,,1,1],1,sum)),3)*100
rownames(tab.r) <- c('&$top$','&$middle$','&$bottom$')
#colnames(tab.r) <- c('$top$','$middle$','$bottom$','Sum')
print(xtable(tab.r,hline = F,digits=1),only.contents=T,include.colnames=F,include.rownames=T,hline.after=NULL,sanitize.text.function = function(x) x)
tab.r <- round(cbind(eps.cont.tab[,,'all','t+4'],Total=apply(eps.cont.tab[,,1,2],1,sum)),3)*100
rownames(tab.r) <- c('&$top$','&$middle$','&$bottom$')
#colnames(tab.r) <- c('$top$','$middle$','$bottom$','Sum')
print(xtable(tab.r,hline = F,digits=1),only.contents=T,include.colnames=F,include.rownames=T,hline.after=NULL,add.to.row=list(pos=list(0),command=c('&&\\multicolumn{4}{c}{$t+4$}\\\\ \n')),sanitize.text.function = function(x) x)
@
