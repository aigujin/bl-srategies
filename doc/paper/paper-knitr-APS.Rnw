...
q.data <- rbind(dt.1[,type:='same'],dt.2[,type:='all'])
...
complete.dt <- na.omit(setkey(rbind(dt.3[,type:='same'],dt.4[,type:='all']),q.id)[quarters])
...
Panel A show the statistics for the consensus expectations as defined in eq. (\ref{consq}). The average consensus among the analysts on stock expected returns in the  \tr{}, \naive{} and \default{} information sets are, respectively, \Sexpr{test.a['Total',,'CONS','mean','all'][[1]]*100 }\%, \Sexpr{test.a['Total',,'CONS','mean','all'][[2]]*100 }\%, and \Sexpr{test.a['Total',,'CONS','mean','all'][[3]]*100}\% respectively. Panel B of the table shows the TP accuracy weighted average expected returns (\Sexpr{test.a['Total',,'PT','mean','all'][[1]]*100 }\%, \Sexpr{test.a['Total',,'PT','mean','all'][[2]]*100 }\%, and \Sexpr{test.a['Total',,'PT','mean','all'][[3]]*100}\%). Panel C shows the EPS based weighted expected returns averages (\Sexpr{test.a['Total',,'EPS','mean','all'][[1]]*100 }\%, \Sexpr{test.a['Total',,'EPS','mean','all'][[2]]*100 }\%, and \Sexpr{test.a['Total',,'EPS','mean','all'][[3]]*100}\%). 
...
\label{sec:perfect}
Panel B presents the results for the case of the \tr{} information set. The annualized cumulative returns for each of the active strategies (\textit{CONS}, \textit{TP}, and \textit{EPS}) are, respectively, \Sexpr{bl.results['CONS',1,1,'all']}\%, \Sexpr{bl.results['TP',1,1,'all']}\%, \Sexpr{bl.results['EPS',1,1,'all']}\%. All the strategies outperform the passive benchmark (\Sexpr{bl.results['Market',1,1,'all']}\%). The results show, as expected, that  we would better off if we knew in advance who the top analysts in terms of TP accuracy would be. In any case, the results suggest that the advice of analysts, as a group, is valuable. 
%Yet, the top analyst  
...
The \textit{TP} ranking strategy dominates also when we look at the risk-adjusted returns. The Sharpe Ratio is \Sexpr{ bl.results['TP',1,'ann.sr','all']} against \Sexpr{ bl.results['CONS',1,'ann.sr','all']}, \Sexpr{ bl.results['EPS',1,'ann.sr','all']}, and \Sexpr{ bl.results['Market',1,'ann.sr','all']}, respectively for the \textit{CONS}, the \textit{EPS}, and the \textit{Market} strategies. In addition, the \textit{TP} strategy dominates  the others if we consider the smaller trading periods (5 years). Table \ref{tab:substrategy} shows the Sharpe Ratio for six 5-year holding periods. The \textit{TP} strategy wins over the others in every period as expected.
...
Panel C of table (\ref{tab:strategy}) shows the performance of the different smart strategies in the \naive{} information set. As it was the  case of the \tr{} information set, all the active strategies outperform the \textit{Market} (\Sexpr{bl.results['Market',2,1,'all']}\%)  and show positive cumulative annualized returns of \Sexpr{bl.results['TP',2,1,'all']}\%,  \Sexpr{bl.results['CONS',2,1,'all']}\%, \Sexpr{bl.results['EPS',2,1,'all']}\% respectively for the \textit{TP}, the \textit{CONS}, and the \textit{EPS} strategies. Similarly, the \textit{TP} strategy dominates when we consider the risk-adjusted returns. The Sharpe Ratio for this strategy is \Sexpr{ bl.results['TP',2,'ann.sr','all']} which is higher than that Sharpe Ratios of the \textit{CONS} and  the \textit{EPS} strategies  (\Sexpr{ bl.results['CONS',2,'ann.sr','all']} and \Sexpr{bl.results['EPS',2,'ann.sr','all']} respectively). The results in panel B of table \ref{tab:substrategy} show as well that this strategy outperform the others for all of 5 trading sub-periods. 

Panel D of table \ref{tab:strategy} confirm the outperformance of the \textit{TP} smart strategy: this is the the only active strategy that yields positive annualized cumulative return (\Sexpr{bl.results['TP',3,1,'all']}\%) whereas both  the \textit{CONS} and the \textit{EPS} show negative returns (\Sexpr{bl.results['CONS',3,1,'all']}\% and \Sexpr{bl.results['EPS',3,1,'all']}\% respectively). The panel C of table \ref{tab:substrategy} shows that the Sharpe Ratio of the \textit{TP} strategy is lower then that of the \textit{EPS} strategy in only first two sub-periods (2000Q1/2004Q4 and 2001Q1/2005Q4).
