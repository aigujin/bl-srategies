...
Panel A shows the dynamics of each tercile for rankings based on target price  accuracy. We observe that analysts exhibit strong ranking consistency as, on average, they stay at the same tercile after one quarter. Of the top (bottom) most accurate (inaccurate) analysts in the previous quarter \Sexpr{pt.cont.tab[[1]][1,1]*100}\% (\Sexpr{pt.cont.tab[[1]][3,3]*100}\%) remain in that same tercile after one quarter. After one year the corresponding figures are lower respectively \Sexpr{pt.cont.tab[[2]][1,1]*100}\% and \Sexpr{pt.cont.tab[[2]][3,3]*100}\% for the top and bottom terciles. 

In the case of EPS ranking  (panel B) \Sexpr{eps.cont.tab[[1]][1,1]*100}\% and  \Sexpr{eps.cont.tab[[1]][3,3]*100}\% ( \Sexpr{eps.cont.tab[[2]][1,1]*100}\% and  \Sexpr{eps.cont.tab[[2]][3,3]*100}\%) of the analysts  remained in the top and bottom terciles after one quarter (year)  respectively.
...
Panel A show the statistics for the consensus expectations as defined in eq. (\ref{consq}). The average consensus among the analysts on stock expected returns in the  \tr{}, \naive{} and \default{} information sets are, respectively, \Sexpr{stat.nr[year=='Total'&Method=='true',mean]*100 }\%, \Sexpr{stat.nr[year=='Total'&Method=='naive',mean]*100 }\%, and \Sexpr{stat.nr[year=='Total'&Method=='default',mean]*100 }\% respectively. Panel B of the table shows the TP accuracy weighted average expected returns (\Sexpr{stat.pt.view[year=='Total'&Method=='true',mean]*100}\%, \Sexpr{stat.pt.view[year=='Total'&Method=='naive',mean]*100}\%, and \Sexpr{stat.pt.view[year=='Total'&Method=='default',mean]*100}\%). Panel C shows the EPS based weighted expected returns averages (\Sexpr{stat.eps.view[year=='Total'&Method=='true',mean]*100}\%, \Sexpr{stat.eps.view[year=='Total'&Method=='naive',mean]*100}\%, and \Sexpr{stat.eps.view[year=='Total'&Method=='default',mean]*100}\%). 
...
The passive strategy generates an annualized cumulative return of \Sexpr{bl.results['Market',1,1]}\% with a Sharpe Ratio  of \Sexpr{bl.results['Market',1,'ann.sr']} over the period 1999-2001. The average number of stocks held per quarter was \Sexpr{bl.results['Market',1,'meanViews']} and the turnover ratio was \Sexpr{bl.results['Market',1,'Ave.TO']}, which reflects solely the inclusion and deletions  of the S\&P 500 constituent list.

\subsection{Perfect foresight strategy}
\label{sec:perfect}
Panel B presents the results for the case of the \tr{} information set. The annualized cumulative returns for each of the active strategies (\textit{CONS}, \textit{TP}, and \textit{EPS}) are, respectively, \Sexpr{bl.results['CONS',1,1]}\%, \Sexpr{bl.results['TP',1,1]}\%, \Sexpr{bl.results['EPS',1,1]}\%. All the strategies outperform the passive benchmark (\Sexpr{bl.results['Market',1,1]}\%). The results show, as expected, that  we would better off if we knew in advance who the top analysts in terms of TP accuracy would be. In any case, the results suggest that the advice of analysts, as a group, is valuable. 
%Yet, the top analyst  
...
The \textit{TP} ranking strategy dominates also when we look at the risk-adjusted returns. The Sharpe Ratio is \Sexpr{ bl.results['TP',1,'ann.sr']} against \Sexpr{ bl.results['CONS',1,'ann.sr']}, \Sexpr{ bl.results['EPS',1,'ann.sr']}, and \Sexpr{ bl.results['Market',1,'ann.sr']}, respectively for the \textit{CONS}, the \textit{EPS}, and the \textit{Market} strategies. In addition, the \textit{TP} strategy dominates  the others if we consider the smaller trading periods (5 years). Table \ref{tab:substrategy} shows the Sharpe Ratio for six 5-year holding periods. The \textit{TP} strategy wins over the others in every period as expected.
...
Panel C of table (\ref{tab:strategy}) shows the performance of the different smart strategies in the \naive{} information set. As it was the  case of the \tr{} information set, all the active strategies outperform the \textit{Market} (\Sexpr{bl.results['Market',2,1]}\%)  and show positive cumulative annualized returns of \Sexpr{bl.results['TP',2,1]}\%,  \Sexpr{bl.results['CONS',2,1]}\%, \Sexpr{bl.results['EPS',2,1]}\% respectively for the \textit{TP}, the \textit{CONS}, and the \textit{EPS} strategies. Similarly, the \textit{TP} strategy dominates when we consider the risk-adjusted returns. The Sharpe Ratio for this strategy is \Sexpr{ bl.results['TP',2,'ann.sr']} which is higher than that Sharpe Ratios of the \textit{CONS} and  the \textit{EPS} strategies  (\Sexpr{ bl.results['CONS',2,'ann.sr']} and \Sexpr{bl.results['EPS',2,'ann.sr']} respectively). The results in panel B of table \ref{tab:substrategy} show as well that this strategy outperform the others for all of 5 trading sub-periods. 

Panel D of table \ref{tab:strategy} confirm the outperformance of the \textit{TP} smart strategy: this is the the only active strategy that yields positive annualized cumulative return (\Sexpr{bl.results['TP',3,1]}\%) whereas both  the \textit{CONS} and the \textit{EPS} show negative returns (\Sexpr{bl.results['CONS',3,1]}\% and \Sexpr{bl.results['EPS',3,1]}\% respectively). The panel C of table \ref{tab:substrategy} shows that the Sharpe Ratio of the \textit{TP} strategy is lower then that of the \textit{EPS} strategy in only first two sub-periods (2000Q1/2004Q4 and 2001Q1/2005Q4).
...
print(xtable(aperm(test.a[12,,,1:3],c(1,3,2))[,,1],display=c('s','f','f','f'),digits=3),only.contents=T,include.colnames=FALSE,hline.after=NULL, format.args=list(big.mark = " ", decimal.mark = "."),add.to.row=list(pos=list(0,0),command=c('\\multicolumn{4}{l}{\\textbf{Panel A: Consensus}} \\\\ \n','\\midrule \n')),sanitize.text.function = function(x) x)
@
\end{tabularx}
...
#cat(rep('&',ncol(results.final)),'\\\\')
print(xtable(aperm(test.a[12,,,1:3],c(1,3,2))[,,2],display=c('s','f','f','f'),digits=3),only.contents=T,include.colnames=FALSE,hline.after=NULL, format.args=list(big.mark = " ", decimal.mark = "."),add.to.row=list(pos=list(0,0),command=c('\\multicolumn{4}{l}{\\textbf{Panel B: TP}} \\\\ \n','\\midrule \n')),sanitize.text.function = function(x) x)
@
  \end{tabularx}
  \begin{tabularx}{\linewidth}{r*{4}{Y}}
...
#cat(rep('&',ncol(results.final)),'\\\\')
print(xtable(aperm(test.a[12,,,1:3],c(1,3,2))[,,3],display=c('s','f','f','f'),digits=3),only.contents=T,include.colnames=FALSE,hline.after=NULL, format.args=list(big.mark = " ", decimal.mark = "."),add.to.row=list(pos=list(0,0),command=c('\\multicolumn{4}{l}{\\textbf{Panel C: EPS}} \\\\ \n','\\midrule \n')),sanitize.text.function = function(x) x)
@
\bottomrule
\end{tabularx}
  \end{table}
...
print(xtable(aperm(test.a[,,c(1,3,2),4],c(1,3,2))[,,1],display=c('s','d','d','d'),digits=3),only.contents=T,include.colnames=FALSE,hline.after=NULL, format.args=list(big.mark = " ", decimal.mark = "."),add.to.row=list(pos=list(0,0,11),command=c('\\multicolumn{4}{l}{\\textbf{Panel A: \\tr{}}} \\\\ \n','\\midrule \n','\\midrule \n')),sanitize.text.function = function(x) x)
@
\end{tabularx}
\begin{tabularx}{\linewidth}{r*{4}{Y}}
<<stocks-naive,echo=F,results='asis'>>=
print(xtable(aperm(test.a[,,c(1,3,2),4],c(1,3,2))[,,2],display=c('s','d','d','d'),digits=3),only.contents=T,include.colnames=FALSE,hline.after=NULL, format.args=list(big.mark = " ", decimal.mark = "."),add.to.row=list(pos=list(0,0,11),command=c('\\multicolumn{4}{l}{\\textbf{Panel B: \\naive{}}} \\\\ \n','\\midrule \n','\\midrule \n')),sanitize.text.function = function(x) x)
@
\end{tabularx}
\begin{tabularx}{\linewidth}{r*{4}{Y}}
<<stocks-def,echo=F,results='asis'>>=
print(xtable(aperm(test.a[,,c(1,3,2),4],c(1,3,2))[,,3],display=c('s','d','d','d'),digits=3),only.contents=T,include.colnames=FALSE,hline.after=NULL, format.args=list(big.mark = " ", decimal.mark = "."),add.to.row=list(pos=list(0,0,11),command=c('\\multicolumn{4}{l}{\\textbf{Panel C: \\default{}}} \\\\ \n','\\midrule \n','\\midrule \n')),sanitize.text.function = function(x) x)
@
\bottomrule
\end{tabularx}
\end{table}
...
print(xtable(t(data.table('\\textit{Market}'=results.final)),display=c('s','f','f','f','d','f'),digits=4,align=c('r',rep('c',length(results.final)))),only.contents=T,include.colnames=FALSE,hline.after=NULL, format.args=list(big.mark = " ", decimal.mark = "."),add.to.row=list(pos=list(0),command=c('\\multicolumn{5}{l}{\\textbf{Panel A}} \\\\ \n')),sanitize.text.function = function(x) x)
cat('\\midrule \n')
@
\end{tabularx}
...
print(xtable(bl.results[1:3,1,],display=c('s','f','f','f','d','f'),digits=4),only.contents=T,include.colnames=FALSE,hline.after=NULL, format.args=list(big.mark = " ", decimal.mark = "."),add.to.row=list(pos=list(nrow(bl.results[1:3,1,])),command=c('\\midrule \n')),sanitize.text.function = function(x) x)
@
\end{tabularx}
...
print(xtable(bl.results[1:3,2,],display=c('s','f','f','f','d','f'),digits=4),only.contents=T,include.colnames=FALSE,hline.after=NULL, format.args=list(big.mark = " ", decimal.mark = "."),,add.to.row=list(pos=list(nrow(bl.results[1:3,2,])),command=c('\\midrule \n')),sanitize.text.function = function(x) x)
@
\end{tabularx}
\begin{tabularx}{\linewidth}{r*{5}{Y}}
  \multicolumn{5}{l}{\textbf{Panel D: \default{}}} \\
<<bl-eps,echo=F,results='asis'>>=
print(xtable(bl.results[1:3,3,],display=c('s','f','f','f','d','f'),digits=4),only.contents=T,include.colnames=FALSE,hline.after=NULL, format.args=list(big.mark = " ", decimal.mark = "."),sanitize.text.function = function(x) x)
@
\bottomrule
\end{tabularx}
\end{table}
...
periods.array[,,,,1] <- periods.array[,,,,1]*100
...
print(xtable(acast(melt(periods.array[,c(4,1,2,3),1,'yes',c(1,4)]),Var1~Var2+Var3,value.var='value'),display=c('s','f','f','f','f','f','f','f','f'),digits=4),only.contents=T,include.colnames=F,hline.after=NULL, format.args=list(big.mark = " ", decimal.mark = "."),add.to.row=list(pos=list(0,6),command=c('\\multicolumn{9}{l}{\\textbf{Panel A: \\tr{}}} \\\\\n', '\\midrule \n')))
...
print(xtable(acast(melt(periods.array[,c(4,1,2,3),2,'yes',c(1,4)]),Var1~Var2+Var3,value.var='value'),display=c('s','f','f','f','f','f','f','f','f'),digits=4),only.contents=T,include.colnames=F,hline.after=NULL, format.args=list(big.mark = " ", decimal.mark = "."),add.to.row=list(pos=list(0,6),command=c('\\multicolumn{9}{l}{\\textbf{Panel B: \\naive{}}} \\\\\n', '\\midrule \n')))
...
print(xtable(acast(melt(periods.array[,c(4,1,2,3),3,'yes',c(1,4)]),Var1~Var2+Var3,value.var='value'),display=c('s','f','f','f','f','f','f','f','f'),digits=4),only.contents=T,include.colnames=F,hline.after=NULL, format.args=list(big.mark = " ", decimal.mark = "."),add.to.row=list(pos=list(0,6),command=c('\\multicolumn{5}{l}{\\textbf{Panel C: \\default{}}} \\\\\n','\\midrule \n')))
@
\bottomrule
\end{tabular}}
\end{table}
