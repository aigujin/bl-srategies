...
\label{sec:perfect}
Panel B presents the results for the case of the \tr{} information set. The annualized cumulative returns for each of the active strategies (\textit{CONS}, \textit{TP}, and \textit{EPS}) are, respectively, \Sexpr{bl.results['CONS',1,1,sel.s]}\%, \Sexpr{bl.results['TP',1,1,sel.s]}\%, \Sexpr{bl.results['EPS',1,1,sel.s]}\%. All the strategies outperform the passive benchmark (\Sexpr{bl.results['Market',1,1,sel.s]}\%). The results show, as expected, that  we would better off if we knew in advance who the top analysts in terms of TP accuracy would be. In any case, the results suggest that the advice of analysts, as a group, is valuable. 
%Yet, the top analyst  
